...
GoodsList = React.createClass {
	...
	componentDidMount: ->
		...
		GoodsAction.getGoodsList 1,_pageSize,'','',''	
		_isBusy = true
	...
	_subMenu1Click :(index) ->
		...
		GoodsAction.getGoodsList _pageNow,_pageSize,stu,pt,ct
		_isBusy = true
		_hasMore = true
	...
	_toGoodsDetail: (index) ->
		goodsId = @state.goodsList[index].id
	...
	render : ->
		...
		listItem = @state.goodsList.map (item, i) ->
			_statusText = ''
		...
		<div>
			<div className="viewport">
				...
				<div style={{display: if _showNoResult then 'block' else 'none'}} className="m-searchNoresult">

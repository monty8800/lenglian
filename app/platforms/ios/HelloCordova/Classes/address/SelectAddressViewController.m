...
-(void)select:(BMKAddressComponent *)address coor:(CLLocationCoordinate2D)coor {
    NSString *userProps = [NSString stringWithFormat: @"{geoAddress:'%@%@%@', geoStreet:'%@%@', lati: '%f', longi: '%f'}", address.province, address.city, address.district, address.streetName, address.streetNumber, coor.latitude, coor.longitude];

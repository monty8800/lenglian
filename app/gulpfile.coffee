...
gulp.task 'serve', ['webpack-dev'], plugins.shell.task('ps -ef |grep "node app.js"|grep -Ev "grep"|awk \'{print $2}\'|xargs kill -9 && node app.js')

gulp.task 'todo', plugins.shell.task('grep -r "TODO" src -n > TODO.md')

gulp.task 'build', ['static', 'sass', 'webpack', 'todo'], plugins.shell.task('cp bower_components/xe-common/js/mobileUtil.js www/util.js')

gulp.task 'coffee', ->
	gulp.src('src/coffee/test/*.coffee').pipe(plugins.coffee()).pipe(gulp.dest('test/'))

gulp.task 'test', ['coffee'], ->
	gulp.src('test/*.js').pipe(plugins.mocha())
